...
#### 功能描述
用户创建个性化的角色档案，这些信息将影响 AI 生成的剧情走向。系统支持多种人生状态，从学生到在职、创业、待业等，提供全面的角色定制能力。
...
- **姓名**（name）: 字符串，必填
- **性别**（gender）: 男/女
- **年龄**（age）: 数字，范围 18-65 岁
- **籍贯**（hometown）: 省市结构化数据（province + city）
- **当前所在地**（currentLocation）: 省市结构化数据
...
- **高校层次**（universityTier）: 仅当年级为大学/研究生时显示
  - Top 2（清北）
  - C9/华五
  - 985/211 重点大学
  - 普通一本/二本
...
**职业与专业**
- **职业**（profession）: 非学生状态时填写，支持智能自动补全
  - 示例：软件工程师、医生、教师
- **专业**（major）: 学生状态时填写，支持智能自动补全
  - 示例：计算机科学、临床医学、金融学
...
- **决策方式**: T（思考）/ F（情感）
- **生活方式**: J（判断）/ P（感知）
- 实时显示 MBTI 类型（如 INTJ）和对应人格名称（如"建筑师"）

**能力与背景**
- **特长与技能**（skills）: 字符串，必填
  - 示例：编程、写作、设计、外语等
- **自定义备注**（customBio）: 可选，用于补充个人故事或特殊设定

#### 技术实现
- 使用 React State 管理表单数据
- 动态表单：根据"当前状态"和"年级"显示/隐藏相关字段
- 智能组件：
  - `LocationCascader`: 省市级联选择器
  - `ProfessionAutocomplete`: 职业/专业智能补全
- 表单验证：
  - 必填字段检查（姓名、籍贯、所在地、技能）
  - 条件必填（学生需填专业和年级，非学生需填职业）
  - 大学生需选择高校层次
- 自动聚焦到姓名输入框，优化用户体验
- 响应式布局：支持桌面和移动端
